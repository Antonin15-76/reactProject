...
export const yearResolve = async (pole, _, ctx) => pole.year
export const driverResolve = async (pole, _, ctx) => pole.driver
export const teamResolve = async (pole, _, ctx) => pole.team
export const raceResolve = async (pole, _, ctx) => pole.race

export const poleResolve = async (root, { id }, ctx) => {
  if (!id) return null
  console.log(ctx.loaders)
  const pole = await loadFromLoader(ctx.loaders.poleLoader, id)
  if (!pole) throw new NotFoundError('Pole not found', { poleId: id })
  return pole
}

export const polesResolve = async (root, { input = {}, count = false }, ctx) => {
...
const poleValidity = async (doc, ctx) => {
  const objectToPole = {
    year: doc.year,
    driver: doc.driver,
    race: doc.race,
  ...
  const result = schema.validate(objectToPole)
  ...
  const poleDocument = await ctx.db.collection(collectionName).findOne(findObj)
  if (poleDocument) throw new UniqueError('', { value: doc, otherValue: poleDocument })

  return poleDocument
}

const preCreate = async (doc, ctx) => {
  return poleValidity(doc, ctx)
}

const postCreate = async (id, ctx) => {
  return poleResolve(null, { id }, ctx)
}

export const createPoleResolve = async (root, {input}, ctx) => {
  const collection = await ctx.db.collection(collectionName)
  ...
  const poleDocument = await preCreate(objectToInsert, ctx)
  if (poleDocument) postCreate(poleDocument._id, ctx)
  let id
  try {
